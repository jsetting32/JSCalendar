...
- (id)initWithFrame:(CGRect)frame {
    return [self initWithFrameAndDirection:frame direction:DMLazyScrollViewDirectionHorizontal circularScroll:NO paging:YES];
}

- (id)initWithFrameAndDirection:(CGRect)frame direction:(DMLazyScrollViewDirection)direction circularScroll:(BOOL) circularScrolling paging:(BOOL)paging {
...
- (void)resetAutoPlay {
    if(_autoPlay) {
        ...
        _timer_autoPlay = [NSTimer scheduledTimerWithTimeInterval:_autoPlayTime target:self selector:@selector(autoPlayHandle:) userInfo:nil repeats:YES];
    } else {
        if(_timer_autoPlay) {
            [_timer_autoPlay invalidate];
            _timer_autoPlay = nil;
...
-(void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
    self.bounces = YES;
    if (controlDelegate != nil && [controlDelegate respondsToSelector:@selector(lazyScrollViewDidEndDragging:)])
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    [self autoPlayPause];
    if (controlDelegate != nil && [controlDelegate respondsToSelector:@selector(lazyScrollViewWillBeginDragging:)])
        [controlDelegate lazyScrollViewWillBeginDragging:self];
}

- (void) scrollViewDidScroll:(UIScrollView *)scrollView {
    if (isManualAnimating) {
        if (controlDelegate != nil && [controlDelegate respondsToSelector:@selector(lazyScrollViewDidScroll:at:withSelfDrivenAnimation:)]) 
    ...
    else if (controlDelegate != nil && [controlDelegate respondsToSelector:@selector(lazyScrollViewDidScroll:at:)])
...
- (void) moveByPages:(NSInteger) offset animated:(BOOL) animated {
    ...
    DMLazyScrollViewTransition transition = (offset >= 0) ? DMLazyScrollViewTransitionForward : DMLazyScrollViewTransitionBackward;
...
- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView {
    if (controlDelegate != nil && [controlDelegate respondsToSelector:@selector(lazyScrollViewWillBeginDecelerating:)])
        [controlDelegate lazyScrollViewWillBeginDecelerating:self];
}

- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (controlDelegate != nil && [controlDelegate respondsToSelector:@selector(lazyScrollViewDidEndDecelerating:atPageIndex:)])
